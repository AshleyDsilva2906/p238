...
#Function for adding address of respective users
def add_address():
    try:
...
#Function to create user's orders
def create_order():
    try:
        user_email = session.get("email")
        user_query = f"select * from users where email='{user_email}';"
...
#Fetching data from user from help page
def submit_help():
...
#Fetch data from search order page
def search_order():
